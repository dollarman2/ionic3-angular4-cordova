...
export class EditDataPage {
  ...
  getCurrentData(rowid) {
    this.sqlite.create({
      ...
      db.executeSql('SELECT * FROM data_records WHERE rowid=?', [rowid])
        .then(res => {
          if(res.rows.length > 0) {
            this.user.rowid = res.rows.item(0).rowid;
            this.user.name = res.rows.item(0).name;
            this.user.age = res.rows.item(0).age;
            this.user.address = res.rows.item(0).address;
            this.user.landmark = res.rows.item(0).landmark;
            this.user.phone = res.rows.item(0).phone;
            this.user.m_status = res.rows.item(0).m_status;
            this.user.husband_name = res.rows.item(0).husband_name;
            this.user.husband_phone = res.rows.item(0).husband_phone;
            this.user.religion = res.rows.item(0).religion;
            this.user.religion_denomination = res.rows.item(0).religion_denomination;
            this.user.menstral_period = res.rows.item(0).menstral_period;
            this.user.first_pregnancy = res.rows.item(0).first_pregnancy;
            this.user.last_child_age = res.rows.item(0).last_child_age;
            this.user.antenatal_during_last_pregnancy = res.rows.item(0).antenatal_during_last_pregnancy;
            this.user.last_child_dlvry_location = res.rows.item(0).last_child_dlvry_location;
            this.user.antenatal_reg_for_pregnancy = res.rows.item(0).antenatal_reg_for_pregnancy;
            this.user.antenatal_reg_facility = res.rows.item(0).antenatal_reg_facility;
            this.user.antenatal_reg_next_schedule = res.rows.item(0).antenatal_reg_next_schedule;
            this.user.antenatal_reg_why = res.rows.item(0).antenatal_reg_why;
            this.user.fam_form_before = res.rows.item(0).fam_form_before;
            this.user.baby_birthday = res.rows.item(0).baby_birthday;
            this.user.baby_delivery_loctn = res.rows.item(0).baby_delivery_loctn;
            this.user.baby_post_natal_checkup = res.rows.item(0).baby_post_natal_checkup;
            this.user.baby_birth_reg = res.rows.item(0).baby_birth_reg;
            this.user.baby_birth_cert = res.rows.item(0).baby_birth_cert;
            this.user.baby_immunization_since_birth = res.rows.item(0).baby_immunization_since_birth;
            this.user.baby_birth_reg_day = res.rows.item(0).baby_birth_reg_day;
            this.user.baby_immunization_card_avail = res.rows.item(0).baby_immunization_card_avail;
            this.user.baby_next_immun_schedule_date = res.rows.item(0).baby_next_immun_schedule_date;
            this.user.baby_vitamin_a_sup = res.rows.item(0).baby_vitamin_a_sup;
          }
        })
        .catch(e => {
          console.log(e);
